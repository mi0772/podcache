...
int main(void) {
    ...
    char *value1 = "Questa è una stringa di test!";

    // Put stringa
    int result1 = cas_put(key1, value1, strlen(value1) + 1); // +1 per null terminator
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (struct_get == 0) {
            ...
            printf("GET Product %d: ✅ (ID: %d, Nome: %s, Prezzo: %.2f)\n",
                   products[i].id, recovered->id, recovered->name, recovered->price);
            free(product_data);
        } else {
            printf("GET Product %d: ❌ ERRORE\n", products[i].id);
        }
    }
    ...
    printf("PUT con chiave NULL: %s\n", null_key_result != 0 ? "✅ ERRORE GESTITO" : "❌ DOVREBBE FALLIRE");
    ...
    printf("GET chiave inesistente: %s\n", missing_result != 0 ? "✅ ERRORE GESTITO" : "❌ DOVREBBE FALLIRE");
    ...
    printf("⚠️  Gestione errori\n");
    printf("\n🚀 CAS System è operativo!\n");
